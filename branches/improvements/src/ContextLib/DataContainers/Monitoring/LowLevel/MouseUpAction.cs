...
{
    public class MouseUpAction : MouseAction
    {
        #region Properties
        ...
        public override string IpySnippet
        {
            get
            {
                return
                    ...
                    "action = ContextLib.DataContainers.Monitoring.MouseUpAction(window, System.Windows.Forms.MouseButtons." + _button + ", " + _x + ", " + _y + ", " + ModifiersIpySnippet + ")" + Environment.NewLine +
        ...
        #region Constructors
        public MouseUpAction(MouseButtons button, uint x, uint y, Modifiers modifiers)
            : base(modifiers)
        {
            _button = button;
            _x = x;
            _y = y;
            _description = "Release " + button.ToString() + " mouse button (Modifiers: " + modifiers + ") at (" + _x + "," + _y + ") on window \"" + _window.Title + "\" (" + _window.ProcessName + ".exe)";
            _quick_description = "MouseUp(" + button.ToString() + ", " + x.ToString() + ", " + y.ToString() + ", " + ((int)modifiers).ToString() + ")"; 
            _type = UserActionType.MouseUpAction;
        }

        public MouseUpAction(Window window, MouseButtons button, uint x, uint y, Modifiers modifiers)
        ...
        {
            UserAction ret = null;
            if (action != null && action.ActionType == UserActionType.MouseDownAction)
            {
                MouseDownAction m_action = (MouseDownAction)action;
                if (this.Button == m_action.Button)
                {
                    if (this.Window.Handle == m_action.Window.Handle &&
                        ...
                        this.Y >= m_action.Y - MAX_Y_OFFSET)
                    {
                        ret = new MouseClickAction(m_action.Window, m_action.Button, m_action.X, m_action.Y, m_action.Modifiers);
                    }
                    else
                    {
                        ret = new MouseDragAction(m_action.Window, m_action.Button, m_action.X, m_action.Y, this.X, this.Y, m_action.Modifiers);
                    }
                    ret.Time = this._time;
                }
            }
