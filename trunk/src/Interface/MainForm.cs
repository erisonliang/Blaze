...
namespace Blaze
{
    public partial class MainForm : Form
    {
        ...
        public MainForm(MainApplication app)
        {
            ...
            _last_assistant_hotkey_press = DateTime.MinValue;
        ...
        {
            if (_assistant_hotkey != null && e.KeyCode == (Keys)_assistant_hotkey.Key)
            {
                DateTime now = DateTime.Now;
                bool update = true;
                if (_last_assistant_hotkey_press != DateTime.MinValue)
                {
                    if (now - _last_assistant_hotkey_press <= new TimeSpan(500 * TimeSpan.TicksPerMillisecond))
                    {
                        ShowAssistantWindow();
                        update = false;
                    }
                ...
                if (update)
                    _last_assistant_hotkey_press = now;
                else
                    _last_assistant_hotkey_press = DateTime.MinValue;
            }
            else
        ...
        //        e.SuppressKeyPress = true;
        //    }
        //    HotKey key = SettingsManager.Instance.GetMainHotKey();
        ...
        public void RegisterAssistantHotKey()
        {
            HotKey hotKey = SettingsManager.Instance.GetAssistantHotKey();
            RegisterAssistantHotKey(hotKey);
        }

        public void RegisterAssistantHotKey(HotKey hotKey)
        {
            if ((Keys)hotKey.Key == Keys.None)
                _assistant_hotkey = null;
            else
                _assistant_hotkey = hotKey;
        }
