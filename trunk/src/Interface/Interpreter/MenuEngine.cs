...
namespace Blaze.Interpreter
{
    public class MenuEngine : InterpreterPlugin
    {
        ...
        {
            Commands.Find(delegate(Command cmd)
            {
                return cmd.Name == Path.GetFileNameWithoutExtension(e.OldFullPath);
            }).SetNameDelegate(new Command.EvaluationDelegate(delegate(string parameters)
                {
                    return Path.GetFileNameWithoutExtension(e.FullPath);
                }));
        }
        ...
        protected override void SetupCommands()
        {
            ...
            {
                int iterations;
                return Int32.TryParse(parameters, out iterations) && iterations > 0;
            }));
            ...
            {
                List<string> args = new List<string>(new string[] { "number of iterations (> 0)" });
                Dictionary<string, bool> comp = new Dictionary<string, bool>();
                foreach (string arg in args)
                    comp.Add(arg, false);
                ...
                int iterations;
                if (Int32.TryParse(parameters, out iterations) && iterations > 0)
                    comp["number of iterations (> 0)"] = true;
