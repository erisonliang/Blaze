...
{
    public class FileMoveGeneralization : Generalization
    {
        #region Properties
        private string _expression;
        private Function[] _functions;
        private string _origin_folder;
        ...
        public string OriginFolder { get { return _origin_folder; } }
        public string DestinationFolder { get { return _destination_folder; } }
        #endregion

        #region Constructors
        public FileMoveGeneralization(string expression, Function[] functions, string origin, string destination, TimeSpan time, int occurrences)
            : base(GeneralizationType.FileMoveGeneralization, time, occurrences)
        {
            _expression = expression;
            _functions = functions;
            _origin_folder = origin;
            _destination_folder = destination;
        }
        #endregion

        #region Public Methods
        public static Generalization[] Generate(string old_folder1, string old_folder2, string new_path1, string new_path2, TimeSpan time)
        {
            ...
            {
                is_file = false;
                folder1 = Directory.GetParent(new_path1).FullName;
                folder2 = Directory.GetParent(new_path2).FullName;
            }
            else
            {
                is_file = true;
                folder1 = Path.GetDirectoryName(new_path1);
                folder2 = Path.GetDirectoryName(new_path2);
            }

            if (folder1 == folder2 && old_folder1 == old_folder2)
            {
                ...
                folder1 = old_folder1;
                string file1 = Path.GetFileNameWithoutExtension(new_path1);
                string file2 = Path.GetFileNameWithoutExtension(new_path2);
                ...
                if (is_file)
                {
                    if (Path.GetExtension(new_path1) == Path.GetExtension(new_path2) && Path.GetExtension(new_path1) != string.Empty)
                        extensions.Add(Path.GetExtension(new_path2));
                    extensions.Add(".*");
                }
                ...
                {
                    gens.Add(new FileMoveGeneralization(expression, sifs.ToArray(), folder1, folder2, time, 2));
                }
                // SequentialCharFunction
                if (SequentialCharFunction.Generate(file1, file2, out expression, out scfs))
                {
                    gens.Add(new FileMoveGeneralization(expression, scfs.ToArray(), folder1, folder2, time, 2));
                }
                // ConstantFileFunction
                if (ConstantFileFunction.Generate(file1, file2, folder1, extensions.ToArray(), out expression, out cff))
                {
                    gens.Add(new FileMoveGeneralization(expression, cff.ToArray(), folder1, folder2, time, 2));
                }
                // ConstantFileFunctionEx
                if (ConstantFileFunctionEx.Generate(file1, file2, folder1, extensions.ToArray(), out expression, out cffe))
                {
                    gens.Add(new FileMoveGeneralization(expression, cffe.ToArray(), folder1, folder2, time, 2));
                }
                // ConstantFileExtFunction
                if (ConstantFileExtFunction.Generate(folder1, extensions.ToArray(), out expression, out cfef))
                {
                    gens.Add(new FileMoveGeneralization(expression, cfef.ToArray(), folder1, folder2, time, 2));
                }
                // ConstantTextFunction
                if (ConstantTextFunction.Generate(file1, file2, out expression, out ctfs))
                {
                    gens.Add(new FileMoveGeneralization(expression, ctfs.ToArray(), folder1, folder2, time, 2));
                }
            }
            return gens.ToArray();
        }
        ...
        public static Generalization[] Merge(Generalization[] prev_gens, Generalization[] new_gens)
        {
            ...
            {
                FileMoveGeneralization prev_gen;
                FileMoveGeneralization new_gen;
                ...
                {
                    prev_gen = (FileMoveGeneralization)prev_gens[i]; // mark the actual old generalization
                    ...
                    {
                        new_gen = (FileMoveGeneralization)new_gens[j]; // mark the actual new generalization
                        ...
                        if (prev_gen.OriginFolder == new_gen.OriginFolder &&
                            prev_gen.DestinationFolder == new_gen.DestinationFolder)
                        {
                            switch (prev_func_type)
                            {
                                        ...
                                        case Function.FunctionType.ConstantFileFunction: // if new generalization has a constant file function type
                                            {
                                                if (prev_gen.Functions.Length == new_gen.Functions.Length)
                                                {
                                                    ...
                                                    if (add_functions)
                                                        gens.Add(new FileMoveGeneralization(new_gen.Expression, new_gen.Functions, new_gen.OriginFolder, new_gen.DestinationFolder, new_gen.Time, prev_gen.Occurrences + 1));
                                                    //else
                                ...
                                case Function.FunctionType.SequentialIntFunction: // if old generalization has a sequential integer function type
                                    switch (new_func_type)
                                    {
                                        ...
                                        case Function.FunctionType.ConstantTextFunction: // if new generalization has a constant text function type
                                            {
                                                if (new_gen.OriginFolder == new_expression)
                                                {
                                                    ...
                                                    gens.Add(new FileMoveGeneralization(prev_gen.Expression, prev_gen.Functions, prev_gen.OriginFolder, prev_gen.DestinationFolder, new_gen.Time, prev_gen.Occurrences + 1));
                                                }
                                            }
                                            break;
                                    }
                                ...
                                case Function.FunctionType.ConstantTextFunction: // if old generalization has a constant text function type
                                    switch (new_func_type)
                                    {
                                        case Function.FunctionType.SequentialIntFunction: // if new generalization has a sequential integer function type
                                            {
                                                if (prev_gen.OriginFolder == prev_expression)
                                                {
        ...
        public override bool Equals(object obj)
        {
            ...
            FileMoveGeneralization generalization = (FileMoveGeneralization)obj;
        ...
        public override object Clone()
        {
            ...
            return new FileMoveGeneralization(this.Expression, dup_func, this.OriginFolder, this.DestinationFolder, this.Time, this.Occurrences);
        }
